...
import {
  ...
  Activity, Compass, History, Settings, LogOut, Map,
} from 'lucide-react';
import { useNavigate } from 'react-router-dom';
import { useAuth } from '../../context/AuthContext';
import { getMapReports, getMapDevices, getMapAlerts } from '../../api/mapApi';
...
// ─── SVG icons para marcadores ───────────────────────────────────────────────
// Paths SVG simples (viewBox 0 0 24 24), estilo Lucide
const INCIDENT_PATHS = {
  deslizamiento: 'M3 20 Q6 10 12 12 Q18 14 21 6 M3 20 L21 20',           // slope wave
  grieta:        'M12 3 L10 10 L14 10 L12 17 M8 20 L16 20',               // lightning bolt
  inundacion:    'M3 14 Q7 10 12 14 Q17 18 21 14 M3 18 Q7 14 12 18 Q17 22 21 18', // waves
  derrumbe:      'M4 20 L8 10 L12 14 L16 6 L20 20 Z',                     // falling triangle
  otro:          'M12 12 m-8 0 a8 8 0 1 0 16 0 a8 8 0 1 0-16 0 M12 8 L12 13 M12 16 L12 17', // circle-info
};

function buildReportMarkerSvg(incident_type, urgency_level) {
  const u = URGENCY[urgency_level] || URGENCY.baja;
  const path = INCIDENT_PATHS[incident_type] || INCIDENT_PATHS.otro;
  // Tamaño por criticidad
  const size = urgency_level === 'alta' ? 44 : urgency_level === 'media' ? 38 : 32;
  const r    = size / 2 - 2;
  const cx   = size / 2;
  // Pin shape: circle + triangle tail
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size + 10}" viewBox="0 0 ${size} ${size + 10}">
    <filter id="ds"><feDropShadow dx="0" dy="3" stdDeviation="2.5" flood-color="${u.color}" flood-opacity="0.35"/></filter>
    ...
    <g transform="translate(${cx - 7},${cx - 7}) scale(0.58)" stroke="${u.stroke}" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" fill="none">
      <path d="${path}"/>
    </g>
...
function buildDeviceMarkerSvg(status, hasAlert) {
  const color = hasAlert ? '#8b2020' : (DEVICE_STATUS[status] || '#3a5560');
  const size  = 38;
  // Antenna path
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size + 10}" viewBox="0 0 ${size} ${size + 10}">
    <filter id="ds2"><feDropShadow dx="0" dy="3" stdDeviation="2.5" flood-color="${color}" flood-opacity="0.4"/></filter>
    <rect x="2" y="2" width="${size - 4}" height="${size - 4}" rx="9" fill="${color}" filter="url(#ds2)"/>
    <rect x="2" y="2" width="${size - 4}" height="${size - 4}" rx="9" fill="none" stroke="rgba(255,255,255,0.45)" stroke-width="1.5"/>
    <g transform="translate(8,7)" stroke="white" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" fill="none">
      ...
      <path d="M6 5 Q11 2 16 5"/>
      <path d="M3 3 Q11 -2 19 3"/>
    ...
    <line x1="${size - 5}" y1="2.5" x2="${size - 5}" y2="6" stroke="white" stroke-width="1.5" stroke-linecap="round"/>
    <circle cx="${size - 5}" cy="8" r="0.8" fill="white"/>` : ''}
    <polygon points="${size / 2 - 6},${size - 2} ${size / 2 + 6},${size - 2} ${size / 2},${size + 9}" fill="${color}"/>
  </svg>`;
  return `data:image/svg+xml;charset=utf-8,${encodeURIComponent(svg)}`;
}

// ─── Popup HTML (sin emojis, con SVG inline) ─────────────────────────────────
...
function buildDevicePopup(d, alerts) {
  const sc = DEVICE_STATUS[d.status] || '#3a5560';
  const lastSeen = d.last_seen_at
    ...
    : 'Desconocido';
  const devAlerts = alerts.filter(a => a.device_code === d.code).slice(0, 4);
...
// ─── Sidebar mini (dentro del mapa) ─────────────────────────────────────────
function MapSidebar({ activeTab, setActiveTab }) {
  const { logout } = useAuth();
  const navigate   = useNavigate();

  const TABS = {
    GENERAL: 'general', SOIL: 'soil', VIBRATION: 'vibration',
    MPU: 'mpu', HISTORICAL: 'historical',
  };
  const items = [
    { key: TABS.GENERAL,    label: 'Resumen del terreno',     icon: <LayoutGrid size={16} strokeWidth={2}/> },
    { key: TABS.SOIL,       label: 'Humedad del suelo',       icon: <Droplets size={16} strokeWidth={2}/> },
    { key: TABS.VIBRATION,  label: 'Sismicidad / vibración',  icon: <Activity size={16} strokeWidth={2}/> },
    { key: TABS.MPU,        label: 'Inclinación / movimiento',icon: <Compass size={16} strokeWidth={2}/> },
    { key: TABS.HISTORICAL, label: 'Histórico',               icon: <History size={16} strokeWidth={2}/> },
  ];
    ...
    <aside className="sw-sidebar">
      ...
      <div className="sw-sidebar-scroll">
        ...
        <button className="sw-nav-btn active" type="button">
          <span className="sw-nav-icon"><Map size={16} strokeWidth={2}/></span>
          <span>Mapa interactivo</span>
        </button>

        <div className="sw-sidebar-section-label">Sistema</div>
        <button className="sw-nav-btn" type="button">
          <span className="sw-nav-icon"><Settings size={16} strokeWidth={2}/></span>
  ...
  useEffect(() => {
    ...
    if (showReports) {
      reports.forEach(r => {
        ...
        const icon = L.icon({
          iconUrl:    buildReportMarkerSvg(r.incident_type, r.urgency_level),
        ...
        L.marker([parseFloat(r.latitude), parseFloat(r.longitude)], { icon })
          ...
          .bindPopup(buildReportPopup(r), { maxWidth: 320, className: 'mp-leaflet-popup' })
          .on('click', () => setSelected({ type: 'report', data: r }));
      });
    }
    ...
    if (showDevices) {
      devices.forEach(d => {
        ...
        const icon = L.icon({
          iconUrl:    buildDeviceMarkerSvg(d.status, hasAlert),
          iconSize:   [38, 48],
          iconAnchor: [19, 48],
          popupAnchor:[0, -48],
        });
        L.marker([parseFloat(d.latitude), parseFloat(d.longitude)], { icon })
          .addTo(layerGroup.current)
          .bindPopup(buildDevicePopup(d, alerts), { maxWidth: 340, className: 'mp-leaflet-popup' })
      ...
      <div className="sw-main" style={{ minHeight: '100vh', overflow: 'hidden' }}>
            ...
            <div className="mp-side-panel">
              ...
              <div className="mp-side-panel-body">
                {selected.type === 'report'
                  ? <ReportDetail report={selected.data} />
        ...
        <div className="mp-legend">
          ...
          <div className="mp-legend-item">
            <span className="mp-legend-dot" style={{ background: '#5a7a3a', borderRadius: 3 }}/>
            Activo
          </div>
          <div className="mp-legend-item">
            <span className="mp-legend-dot" style={{ background: '#8b2020', borderRadius: 3 }}/>
            Con alerta
          </div>
        </div>
      </div>
...
// ─── Panel lateral: detalle dispositivo ──────────────────────────────────────
function DeviceDetail({ device, alerts }) {
  const sc  = DEVICE_STATUS[device.status] || '#3a5560';
  ...
  return (
    <div className="mp-detail">
      ...
      {devAlerts.length === 0 ? (
        <div className="mp-detail-no-alerts">
          <CheckCircle size={15}/> Sin alertas activas
        </div>
      ) : (
          ...
          {devAlerts.map(a => (
            <div key={a.id} className="mp-detail-alert" style={{ borderLeft: `3px solid ${ALERT_COLOR[a.level] || '#b7791f'}` }}>
              ...
              {a.current_value != null && (
                <div className="mp-detail-alert-vals">
                  Valor: <b>{a.current_value} {a.unit || ''}</b> · Umbral: {a.threshold_value}
                </div>
              )}
