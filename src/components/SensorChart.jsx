...
function ChartCanvas({ title, data, times, color, threshold, unit, height = 110, isExpanded = false }) {
  const canvasRef = useRef(null);
  const wrapRef = useRef(null);

  const zoomRef = useRef({ start: 0, end: Math.max(data.length - 1, 0), locked: false });
  ...
  const draw = useCallback((s, e, hov) => {
    ...
    for (let v = yMin; v <= yMax + yStep * 0.01; v += yStep) {
      ...
      const lbl = Number.isInteger(v) ? String(v) : parseFloat(v.toFixed(2)).toString();
      ctx.fillStyle = lblColor;
      ctx.fillText(lbl, PAD.l - 7, py + 3);
    }

    // ── X-axis labels: collision-aware ──────────────────────────────────────
    ctx.font = `${fontSize}px 'DM Mono', monospace`;
    ctx.textAlign = "center";

    // Measure label width at 25° rotation so we know min spacing
    const sampleLabel = formatAxisTimeLabel(tslice[0], slice.length, isExpanded) || "00:00";
    const labelW = ctx.measureText(sampleLabel).width;
    // Projected horizontal span when rotated ~25°
    const projectedW = labelW * Math.cos(Math.PI / 7) + fontSize * Math.sin(Math.PI / 7);
    const minGap = projectedW + (isExpanded ? 14 : 10);

    // Build candidate indices: evenly spaced + always include last
    const maxLabels = Math.max(2, Math.floor(cW / minGap));
    const step = Math.max(1, Math.floor((slice.length - 1) / (maxLabels - 1)));

    const candidates = new Set();
    for (let i = 0; i < slice.length; i += step) candidates.add(i);
    candidates.add(slice.length - 1);

    // Remove any index whose pixel position is too close to the previous printed one
    let lastPx = -Infinity;
    const approved = [];
    ...
    if (threshold !== null && threshold !== undefined) {
      ...
      if (ty >= PAD.t - 2 && ty <= PAD.t + cH + 2) {
        ...
        ctx.font = `${isExpanded ? 10 : 8}px 'DM Mono', monospace`;
        ctx.textAlign = "left";
    ...
    if (hov >= 0 && hov < slice.length) {
      ...
      ctx.font = `500 ${isExpanded ? 12 : 10}px 'DM Mono', monospace`;
      const tw = ctx.measureText(lbl).width;
      const bw = tw + 16; const bh = isExpanded ? 26 : 20;
      let bx = hx + 10; let by = hy - 30;
      if (bx + bw > W - 4) bx = hx - bw - 10;
      if (by < PAD.t) by = PAD.t + 4;
      ctx.fillStyle = "#fff"; ctx.strokeStyle = tooltipBorder; ctx.lineWidth = 0.8;
      ctx.beginPath(); ctx.roundRect(bx, by, bw, bh, 6); ctx.fill(); ctx.stroke();
      ctx.fillStyle = "#2a1f15"; ctx.textAlign = "left";
  ...
  useEffect(() => {
    zoomRef.current = { start: 0, end: Math.max(data.length - 1, 0), locked: false };
    hoverRef.current = -1;
    setIsLocked(false); setZoomInfo("");
    renderWithState();
  ...
  const handleMouseMove = useCallback((e) => {
    ...
    if (dragRef.current.active) {
      ...
      ctx.fillRect(sx, PAD.t, ex - sx, cH); ctx.strokeRect(sx, PAD.t, ex - sx, cH);
    } else {
  ...
  const handleMouseLeave = useCallback(() => { hoverRef.current = -1; renderWithState(); }, [renderWithState]);
  const handleMouseDown = useCallback((e) => {
    const canvas = canvasRef.current; if (!canvas) return;
    const rect = canvas.getBoundingClientRect();
    dragRef.current = { active: true, x0: e.clientX - rect.left, selS: 0, selE: 0 };
  }, []);
  ...
  const handleMouseUp = useCallback(() => {
    ...
    if (d.selE - d.selS > 2) {
      ...
      hoverRef.current = -1; setIsLocked(true);
      setZoomInfo(`${formatFullTooltipTime(times[d.selS])} → ${formatFullTooltipTime(times[d.selE])} · ${d.selE - d.selS + 1} muestras`);
      renderWithState();
    } else { renderWithState(); }
  }, [renderWithState, times]);

  const handleDoubleClick = useCallback(() => {
    zoomRef.current = { start: 0, end: Math.max(data.length - 1, 0), locked: false };
    hoverRef.current = -1; setIsLocked(false); setZoomInfo(""); renderWithState();
  }, [data.length, renderWithState]);

  const handleUnlock = useCallback(() => {
    zoomRef.current.locked = false;
    zoomRef.current.start = 0; zoomRef.current.end = Math.max(data.length - 1, 0);
    setIsLocked(false); setZoomInfo(""); renderWithState();
  }, [data.length, renderWithState]);
  ...
  const handleWheel = useCallback((e) => {
    ...
    if (e.shiftKey) {
      ...
      if (e.deltaY > 0) { s = clamp(s + move, 0, Math.max(data.length - currentRange, 0)); ee = s + currentRange - 1; }
      else { s = clamp(s - move, 0, Math.max(data.length - currentRange, 0)); ee = s + currentRange - 1; }
    } else if (e.deltaY < 0) {
      const nextRange = Math.max(6, Math.round(currentRange * 0.75));
      const leftRatio = (centerIdx - s) / Math.max(currentRange - 1, 1);
      s = Math.round(centerIdx - leftRatio * (nextRange - 1)); ee = s + nextRange - 1;
      if (s < 0) { ee += -s; s = 0; }
      if (ee > data.length - 1) { s -= ee - (data.length - 1); ee = data.length - 1; }
      s = Math.max(0, s);
    } else {
    ...
    setIsLocked(locked);
    if (locked) setZoomInfo(`${formatFullTooltipTime(times[s])} → ${formatFullTooltipTime(times[ee])} · ${ee - s + 1} muestras`);
    else setZoomInfo("");
    hoverRef.current = -1; renderWithState();
...
export default function SensorChart({ title, data, times, color, threshold, unit }) {
  const [expanded, setExpanded] = useState(false);

  useEffect(() => {
    const onKeyDown = (e) => { if (e.key === "Escape") setExpanded(false); };
    if (expanded) { window.addEventListener("keydown", onKeyDown); document.body.style.overflow = "hidden"; }
        ...
        <div className="sw-chart-top">
          ...
          >
            <svg width="15" height="15" viewBox="0 0 16 16" fill="none">
              <path d="M6 2H2v4M10 2h4v4M14 10v4h-4M2 10v4h4" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round" />
            </svg>
          </button>
        </div>

        <div onClick={() => setExpanded(true)} className="sw-chart-clickable">
          <ChartCanvas title={title} data={data} times={times} color={color} threshold={threshold} unit={unit} height={220} />
        </div>
      </div>

      {expanded && (
        <div className="sw-chart-modal" onClick={() => setExpanded(false)}>
          <div className="sw-chart-modal-panel" onClick={(e) => e.stopPropagation()}>

            {/* ── Header with close button INSIDE the panel ── */}
            <div className="sw-chart-modal-head">
              ...
              <button
                ...
                aria-label="Cerrar"
                style={{
                  // Make sure it stays inside the panel, not floating outside
                  position: "relative",
                  flexShrink: 0,
            ...
            <ChartCanvas
              title={title} data={data} times={times}
              color={color} threshold={threshold} unit={unit}
              height={430} isExpanded
            />
          </div>
        </div>
      )}
